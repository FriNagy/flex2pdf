...
/*		  int  icopar = 0;
		  ...
		  case 'u': {
			  char cport[32];
			  strncpy(cport, mycoml[endlaufi], 31); cport[31] = '\0';
			  int cplen = strlen(cport);
			  if (cplen > 0 && cport[cplen-1] == ':') cport[cplen-1] = '\0';
			  if (!cport[0]) strcpy(cport, "COM3");
			  if (!OpenDrawer_BREAK_Pulse(cport, 250, 200)) {
				  sprintf(mMsg, "cash_up: COM-Port '%s' nicht erreichbar!\n(Error %lu)", cport, GetLastError());
				  Msg(mMsg);
			  }
			  break;
		  }
...
void MyCmlHelp()
{
 ...
 //  VpeCreateUDO(hDoc, 3.00, 3.00, -3.00, -3.00, UDO_OBJECT_ID);
   ...
   VpeWriteBox(hDoc, 10.00, 10.00, VFREE, VFREE, "[N 'Arial' S 8 SH C RED]Programm mixed by f.nagy v. 2025\nPrint Engine Copyright Ideal-Software");
   VpePreviewDoc(hDoc, NULL , VPE_SHOW_MAXIMIZED);
